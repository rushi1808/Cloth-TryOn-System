...
export async function POST(request) {
    try {
        ...
        const { history, message, outfitContext, closetInventory } = body;

        const res = await fetch(`${PYTHON_API}/chat-stylist`, {
            method: 'POST',
        ...
        if (!res.ok) {
            ...
            console.error('Python service error (chat-stylist):', err);
